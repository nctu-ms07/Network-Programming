...
std::string sock4_host;
std::string sock4_port;
...
class TcpClient : public std::enable_shared_from_this<TcpClient> {
 public:
  ...
  void async_connect(const Console_ARG &console_arg) {
    ...
    resolver_.async_resolve(sock4_host,
                            sock4_port,
                              ...
                              self->socket_.async_connect(it->endpoint(),
                                                          [self, console_arg](beast::error_code) {
                                                            ...
                                                            self->file_.open("test_case/" + console_arg.file,
                                                                             std::fstream::in);
                                                            self->sock4_connect(console_arg.host, console_arg.port);
                                                            self->async_read();
                                                          });
                            });
  }
 ...
 private:
  ...
  void sock4_connect(const std::string &host, const std::string &port) {
    uint8_t version = 4;
    uint8_t code = 1;
    auto destPort = boost::lexical_cast<uint16_t>(port);
    destPort = (destPort >> 8) | (destPort << 8);
    std::array<uint8_t, 4> destIP = {0, 0, 0, 1};
    uint8_t null = 0;

    std::array<boost::asio::const_buffer, 7> request = {
    ...
    std::array<boost::asio::mutable_buffer, 4> respond = {
        boost::asio::buffer(&version, 1),
        boost::asio::buffer(&code, 1),
        boost::asio::buffer(&destPort, 2),
        boost::asio::buffer(destIP),
    };
    boost::asio::read(socket_, respond);
  };
